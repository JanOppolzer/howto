...
_Pokud by nám nevyhovovalo, že všechny instance budou "schovány" pod jedním `entityID` ve federaci, je možné nakonfigurovat Shibboleth SP tak, aby každá instance měla svá vlastní metadata, své vlastní `entityID` a podobně. Po každém přidání takové instance DokuWiki je pak ale potřeba udělat ruční konfigurační změny v `/etc/shibboleth/shibboleth2.xml`, které však nejsou nijak složité. Tato varianta zde NENÍ zdokumentovaná. V případě potřeby se ozvěte na e-mail <jan.oppolzer@cesnet.cz>._
